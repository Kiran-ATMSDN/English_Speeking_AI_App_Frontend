...
import { EnglishLearningTip } from '../../core/models/api.models';
...
@Component({
  selector: 'app-English-learning-tips',
  templateUrl: './english-learning-tips.page.html',
  styleUrls: ['./english-learning-tips.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    IonButton,
...
export class EnglishLearningTipsPage implements OnInit {
  ...
  speakTip(tip: EnglishLearningTip): void {
    const speechText = `Learning tip. ${tip.title}. Description. ${tip.description}. ${
      tip.actionStep ? `Action step. ${tip.actionStep}.` : ''
    }`;
  ...
  speakAll(): void {
    ...
    this.tips.forEach((tip, index) => {
      const utterance = new SpeechSynthesisUtterance(
        `Tip ${index + 1}. ${tip.title}. Description. ${tip.description}. ${
          tip.actionStep ? `Action step. ${tip.actionStep}.` : ''
        }`,
      );
  ...
  private fetchTips(day?: number): void {
    ...
    this.loading = true;
    this.onboardingService.getEnglishLearningTips(requestedDay).subscribe({
      next: (res) => {
      ...
      error: (error) => {
        this.loading = false;
        void this.notificationService.error(getErrorMessage(error, 'Failed to load English learning tips.'));
      },
    });
  }
}
